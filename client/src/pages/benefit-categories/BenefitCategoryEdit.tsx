...
import {
  getListBenefitCategory,
  getBenefitCategory,
  updateBenefitCategory,
} from "../../api/benefit-categories";
...
//css flex box
export const BenefitCategoryEdit = () => {
  ...
  const [formState, setFormState] = useState<BenefitCategory>({
  ...
  const benefitCategoryId = params?.benefitCategoryId ?? null;

  useEffect(() => {
    if (benefitCategoryId) {
      getBenefitCategory(benefitCategoryId).then((result) => {
        setFormState({
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    updateBenefitCategory(formState.id, {
      ...formState,
  ...
  const handleClick = () => {
    ...
    navigate("/benefit-categories/all");
  };
  ...
  return (
    <div className={classes.div}>
      ...
      <Grid container>
        <Grid item xs={9} md={9}>
          <h2 className={classes.headerText}>Edit BenefitCategory</h2>
        </Grid>
        <SubmitButton
          type={"Edit"}
          display={!editMode}
          ...
          <SingleSelect
            ...
            options={
              dataQueryParent.data?.data.data
                ...
                .filter((item) => item.id !== benefitCategoryId) ?? []
            }
          />
          <BooleanSelection
            icon={<ToggleOnIcon />}
